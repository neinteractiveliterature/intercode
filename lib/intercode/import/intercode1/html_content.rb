class Intercode::Import::Intercode1::HtmlContent
  ...
  def import!
    ...
    html_paths.each do |html_path|
      ...
      existing_standard_content = content.class.where(slug: content.slug).first
  ...
  def build_content(html_path)
    ...
    when 'acceptingbids', 'bidding1', 'bidearly', 'copyright', 'logintop', 'loginbottom'
      # The TEXT_DIR contains partials mixed with pages, with no real indication which is which.
      # This is a list of known partials in Intercode 1, which we'll turn into partials here too.
  ...
  # The "html" files inside Intercode 1 are actually fragments of PHP that assume intercon_db.inc
  # has been loaded before they're executed.  So let's do the needful here.
