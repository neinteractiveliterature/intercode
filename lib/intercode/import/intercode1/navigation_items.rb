class Intercode::Import::Intercode1::NavigationItems
  ...
  def import!
    ...
    NAVIGATION_STRUCTURE.each { |item| import_navigation_item(item) }
  end
  ...
  def import_navigation_item(item)
    case item[:type]
    when :section
      import_navigation_section(item)
    when :link
      import_navigation_link(item, nil)
    else
      raise "Unknown navigation item type #{item[:type].inspect}"
    end
  end
  ...
  def import_navigation_section(item)
    ...
    item[:links].each { |link| import_navigation_link(link, section) }
  end
  ...
  def import_navigation_link(item, navigation_section)
    ...
    if page
      ...
      logger.info(
        "Not creating navigation link #{item[:title]} because \
