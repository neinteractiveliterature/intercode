class Intercode::Import::Intercode1::Tables::Con < Intercode::Import::Intercode1::Table
  def initialize(connection, config)
    ...
    friday_start = @timezone.local(friday_date.year, friday_date.month, friday_date.day, 0, 0, 0)
...
to #{friday_start.strftime('%A, %b %d, %Y')}."
  ...
  def update_cms_content(con)
    ...
    news_content =
      Intercode::Import::Intercode1::HtmlConverter.new(html: @row[:News], convention: con, file_root: file_root).convert

    con_com_meetings_content =
