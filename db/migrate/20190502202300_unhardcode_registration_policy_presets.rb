class UnhardcodeRegistrationPolicyPresets < ActiveRecord::Migration[5.2]
  DEFAULT_PRESETS = [
    {
  ...
  def change
    reversible do |dir|
      dir.up do
        FormItem.where(item_type: 'registration_policy').find_each do |form_item|
          next if form_item.properties['presets'].present?
          form_item.update!(
            properties: form_item.properties.merge(
