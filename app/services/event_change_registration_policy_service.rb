class EventChangeRegistrationPolicyService < CivilService::Service
  ...
  class SignupSimulator
    ...
    def simulate_signup(signup)
      ...
      current_bucket = (
        signup.bucket_key ? registration_policy.bucket_with_key(signup.bucket_key) : nil
      )
      destination_bucket = (
...
#{destination_bucket.key} (was #{signup.bucket_key})"
  ...
  def signups_by_run_id
    @signups_by_run_id ||= all_signups.group_by(&:run_id).transform_values do |signups|
      signups.sort_by { |signup| [signup.confirmed? ? 0 : 1, signup.created_at] }
    end
  end
