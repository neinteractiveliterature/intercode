...
class RefreshSslCertificateService < CivilService::Service
  ...
  def install_acme
    ...
    sh "curl https://raw.githubusercontent.com/acmesh-official/acme.sh/master/acme.sh | \
sh -s -- --install-online --force"
  end
...
--issue --server letsencrypt --challenge-alias #{root_domain} --dns dns_cf \
#{domain_args.join(" ")}"
  end
