...
class EventWithdrawService < CivilService::Service
  class Result < CivilService::Result
    attr_accessor :move_results, :prev_bucket_id, :prev_state
  end
  ...
  def initialize(
    ...
    @prev_bucket_id = signup.bucket_id
  ...
  def inner_call
    with_advisory_lock_unless_skip_locking("run_#{run.id}_signups") do
      signup.update!(state: "withdrawn", bucket_id: nil, counted: false, updated_by: whodunit)
      signup.log_signup_change!(action:)

      move_result = move_signups(prev_state, prev_bucket_id, prev_counted)
      return move_result if move_result.failure?
    end

    notify_team_members(signup, prev_state, prev_bucket_id, move_results)
    send_confirmation(signup, prev_state, prev_bucket_id)
    success(prev_state:, prev_bucket_id:, move_results:)
  end

  def notify_team_members(signup, prev_state, prev_bucket_id, move_results)
  ...
  def send_confirmation(signup, prev_state, prev_bucket_id)
  ...
  def move_signups(prev_state, prev_bucket_id, prev_counted)
    return success unless prev_counted && prev_state == "confirmed"
    if prev_bucket_id
      # Don't fill vacancies in a previously-overfilled bucket
      prev_bucket = signup.run.event.registration_policy.bucket_with_id(prev_bucket_id)
      return success if prev_bucket&.full?(signup.run.signups)
    end

    vacancy_fill_result = fill_vacancy(prev_bucket_id)
  ...
  def fill_vacancy(prev_bucket_id)
    EventVacancyFillService.new(run, prev_bucket_id, skip_locking: true).call
  end
