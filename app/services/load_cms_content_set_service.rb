class LoadCmsContentSetService < ApplicationService
  ...
  def inner_call
    all_template_paths_with_names('layouts').each do |path, name|
      cms_layout = @convention.cms_layouts.create!(name: name, content: template_content(path))
  ...
  def ensure_no_conflicting_layouts
    ...
    if all_template_paths_with_names('layouts').map(&:last).include?('Default') && convention.default_layout
