...
class ProvideEventTicketService < CivilService::Service
  ...
  def inner_call
    ...
    with_advisory_lock_unless_skip_locking("event_#{event.id}_provided_tickets") do
      ...
      ticket = event.provided_tickets.create!(ticket_type: ticket_type, user_con_profile: user_con_profile)
    end
  ...
  def ticket_type_must_be_providable
    ...
    errors.add :base,
               "#{ticket_type.name} #{convention.ticket_name.pluralize} cannot be provided \
by #{event.title}"
  end
  ...
  def event_must_be_able_to_provide_tickets
    ...
    errors.add :base,
               "#{event.title} cannot provide #{convention.ticket_name.pluralize} to \
attendees"
  end

  def event_must_have_remaining_tickets_of_type
    return unless maximum_event_provided_tickets_for_event.positive?
...
provided #{already_provided_count}"
  ...
  def user_con_profile_must_not_have_ticket
    ...
    errors.add :base,
               "#{user_con_profile.name} already has a #{convention.ticket_name} to \
