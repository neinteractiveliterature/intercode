  ...
  def read?
    if oauth_scoped_disjunction do |d|
         ...
         d.add(:read_profile) do
           user &&
             CouponApplication
               .joins(order: :user_con_profile)
               .where(coupon_id: record.id, user_con_profiles: { user_id: user.id })
               .any?
         end
       end
      return true
    ...
    def resolve
      ...
      disjunctive_where do |dw|
        if oauth_scope?(:read_conventions)
          dw.add(convention_id: conventions_with_permission('update_products').select(:id))
        end
      end
    end
  end
