...
removed in favor of `conventionByRequestHostIfPresent`." do
  ...
  field :conventions,
        ...
        null: false,
        deprecation_reason:
          "This field is being removed due to its potential performance \
...
Please use the `cmsFiles` field on the CmsParent interface instead." do
  ...
  def cms_files
    context[:convention] ? context[:convention].cms_files : CmsFile.global
  end
...
Please use the `searchCmsContent` field on the CmsParent interface instead." do
  ...
  def cms_pages
    context[:convention] ? context[:convention].pages : Page.global
  end
...
Please use the `cmsLayouts` field on the CmsParent interface instead." do
  ...
  def cms_layouts
    context[:convention] ? context[:convention].cms_layouts : CmsLayout.global
  end
...
Please use the `cmsPartials` field on the CmsParent interface instead." do
  ...
  def cms_partials
    context[:convention] ? context[:convention].cms_partials : CmsPartial.global
  end
...
Please use the `cmsVariables` field on the CmsParent interface instead." do
  ...
  def cms_variables
    context[:convention] ? context[:convention].cms_variables : CmsVariable.global
  end

  field :cms_graphql_queries,
        [Types::CmsGraphqlQueryType],
        null: false,
        deprecation_reason:
          "Domain-specific queries are being deprecated. \
Please use the `cmsGraphqlQueries` field on the CmsParent interface instead." do
  ...
  def cms_graphql_queries
    context[:convention] ? context[:convention].cms_graphql_queries : CmsGraphqlQuery.global
  end
...
Please use the `cmsNavigationItems` field on the CmsParent interface instead." do
  ...
  def cms_navigation_items
    context[:convention] ? context[:convention].cms_navigation_items : CmsNavigationItem.global
  end

  field :effective_cms_layout,
        Types::CmsLayoutType,
        null: false,
        deprecation_reason:
          "Domain-specific queries are being deprecated. \
...
Please use the `preview_notifier_liquid` field on the Convention type instead." do
    ...
    authorize { |_value, _args, context| Pundit.policy(context[:pundit_user], assert_convention).view_reports? }
  end
...
Please use the `current_pending_order` field on the UserConProfile type instead." do
  ...
  field :current_pending_order_payment_intent_client_secret,
        String,
        null: false,
        deprecation_reason:
          "Domain-specific queries are being deprecated. \
      ...
      Stripe::PaymentIntent.create(
        {
          ...
          metadata: {
            order_id: current_pending_order.id
          }
        },
