class Mutations::ConvertTicketToEventProvided < GraphQL::Schema::RelayClassicMutation
  graphql_name 'ConvertTicketToEventProvided'
  field :deleted_ticket, Types::TicketType,
    'The ticket we deleted in order to provide a new ticket',
    null: false, camelize: false
  field :refund_status, Types::RefundStatusType, null: false, camelize: false
  ...
  def resolve(event_id:, ticket_type_id:, user_con_profile_id:)
    event = context[:convention].events.find(event_id)
    ticket_type = context[:convention].ticket_types.find(ticket_type_id)
    user_con_profile = context[:convention].user_con_profiles.find(user_con_profile_id)
    existing_ticket = user_con_profile.ticket
    raise "#{user_con_profile.name_without_nickname} has no #{context[:convention].ticket_name}" unless existing_ticket
