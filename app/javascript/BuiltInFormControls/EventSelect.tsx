...
type DO<QueryType extends DefaultEventsQueryQuery> = (
  NonNullable<QueryType['convention']>['events_paginated']['entries'][0]
);

export type EventSelectProps<DataType, OptionType extends OptionTypeBase> = (
...
function EventSelect<DataType extends DQ = DQ, OptionType extends DO<DataType> = DO<DQ>>(
  { eventsQuery, ...otherProps }: EventSelectProps<DataType, OptionType>,
) {
  return (
    <GraphQLAsyncSelect<DataType, OptionType>
      isClearable
      getOptions={(data) => (
        (data.convention?.events_paginated?.entries ?? []) as OptionType[]
      )}
