...
function ConventionDayTabContainer({
  conventionTimespan, timezoneName, prefetchTimespan, children,
}) {
  ...
  const conventionDayTabs = conventionDayTimespans.map((timespan) => {
    ...
    return (
      <li className="nav-item" key={timespan.start.toISOString()}>
        <NavLink
          to={`/${timespan.start.format('dddd').toLowerCase()}`}
          className="nav-link"
          {...prefetchProps}
        >
  ...
  return (
    <div>
      ...
      <Switch>
        {conventionDayRoutes}
        <Redirect to={`${conventionDayTimespans[0].start.format('dddd').toLowerCase()}`} />
      </Switch>
    </div>
  );
}
