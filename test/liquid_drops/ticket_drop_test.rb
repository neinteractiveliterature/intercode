...
describe TicketDrop do
  ...
  let(:event) { FactoryGirl.create(:event, convention: convention) }
  let(:ticket) { FactoryGirl.create(:ticket, user_con_profile: user_con_profile, provided_by_event: event) }
