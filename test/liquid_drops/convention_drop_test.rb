...
describe ConventionDrop do
  ...
  describe 'with runs that have openings' do
    let(:event_with_openings) { FactoryGirl.create(:event, convention: convention) }
    let(:run_with_openings) { FactoryGirl.create(:run, event: event_with_openings) }
    let(:volunteer_event_with_openings) { FactoryGirl.create(:event, convention: convention, category: 'volunteer_event') }
    let(:volunteer_run_with_openings) { FactoryGirl.create(:run, event: volunteer_event_with_openings) }
    let(:event_without_openings) { FactoryGirl.create(:event, convention: convention, registration_policy: RegistrationPolicy.new) }
    let(:run_without_openings) { FactoryGirl.create(:run, event: event_without_openings) }
