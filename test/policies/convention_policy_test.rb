...
class ConventionPolicyTest < ActiveSupport::TestCase
    ...
    describe "##{action}?" do
      describe "when #{field} is 'yes'" do
        it "lets anyone #{action}" do
          convention = FactoryBot.create(:convention, field => 'yes')
          assert ConventionPolicy.new(nil, convention).public_send("#{action}?")
        end
      end

      describe "when #{field} is 'gms'" do
        let(:convention) { FactoryBot.create(:convention, field => 'gms') }

        it "lets team members #{action}" do
          event = FactoryBot.create(:event, convention: convention)
      ...
      describe "when #{field} is 'priv'" do
        let(:convention) { FactoryBot.create(:convention, field => 'priv') }
      ...
      describe "when #{field} is 'no'" do
        let(:convention) { FactoryBot.create(:convention, field => 'no') }
  ...
  describe '#schedule_with_counts?' do
    %w[priv gms yes no].each do |value|
      describe "when show_schedule is '#{value}'" do
        let(:convention) { FactoryBot.create(:convention, show_schedule: value) }
  ...
  describe '#update?' do
    it 'lets staff users update' do
      user_con_profile = FactoryBot.create(:user_con_profile, convention: convention, staff: true)
  ...
  describe 'Scope' do
    it 'lets anyone read any convention' do
      conventions = FactoryBot.create_list(:convention, 3)
