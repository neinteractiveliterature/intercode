...
class FormPolicyTest < ActiveSupport::TestCase
  ...
  describe '#manage?' do
    it 'lets con staff manage forms' do
      form = FactoryBot.create(:form)
      user_con_profile = FactoryBot.create(:staff_user_con_profile, convention: form.convention)
      assert FormPolicy.new(user_con_profile.user, form).manage?
    end

    it 'does not let non-staff manage forms' do
      form = FactoryBot.create(:form)
      user_con_profile = FactoryBot.create(:user_con_profile, convention: form.convention)
