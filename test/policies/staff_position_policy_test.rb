...
class StaffPositionPolicyTest < ActiveSupport::TestCase
  describe '#read?' do
    it 'lets anyone read any staff position' do
      staff_position = FactoryBot.create(:staff_position)
  ...
  describe '#manage?' do
    ...
    it 'does not let non-staff manage staff_positions' do
      staff_position = FactoryBot.create(:staff_position)
      user_con_profile = FactoryBot.create(:user_con_profile, convention: staff_position.convention)
  ...
  describe 'Scope' do
    it 'always returns all staff_positions' do
      FactoryBot.create_list(:staff_position, 3)
