...
React Router loaders and actions run during navigation and need to access the client from the router context. Use React Router v7's type-safe routing with auto-generated route types.
...
- Import route types from `./+types/route` (auto-generated by React Router v7)
- Use `Route.ClientLoaderArgs` and `Route.ClientActionArgs` for type-safe parameters
- Export `clientLoader` and `clientAction` (not `loader` and `action`)
- Get client with `context.get(apolloClientContext)`
- TypeScript automatically infers types from query documents - no need for explicit generics
- Import `apolloClientContext` from `'~/AppContexts'`
...
❌ **Don't**: Use old manual type annotations
...
export const clientLoader: LoaderFunction<RouterContextProvider> = async ({ context }) => {
  // Use Route.ClientLoaderArgs instead
};
```
...
1. Use React Router v7's type-safe routing with `Route.ClientLoaderArgs` and `Route.ClientActionArgs`
2. Import route types from `./+types/route` (auto-generated)
3. Always get the client from context in loaders/actions
4. Let TypeScript infer types from query documents - avoid explicit generics
5. Run `yarn run tsc --noEmit` to check for TypeScript errors
6. Test actual navigation flows to ensure data loading works
...
**Solution**: Remove `useApolloClient` import, add `apolloClientContext` import, import route types from `./+types/route`, and use `Route.ClientLoaderArgs`.

### "Cannot find module './+types/route'"

**Cause**: Route types are auto-generated by React Router v7 and may not exist yet.
**Solution**: Run the typegen comand (`yarn run react-router typegen`) to generate the types.

### "Type 'LoaderFunction<RouterContextProvider>' is not assignable..."

**Cause**: Using old manual type annotations instead of React Router v7's type-safe routing.
**Solution**: Replace `LoaderFunction<RouterContextProvider>` with `Route.ClientLoaderArgs` from `./+types/route`.
